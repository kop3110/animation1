...
class Scatter_animation(object):
	def __init__(self, sleep=False):
		...
		self.mode = 1
		self.ylist = [(self.x1, self.y1), (self.x1, self.y2), (self.x1, self.y3),(self.x1, self.y5),(self.x1, self.y6)]
		self.clist = [0.2, 0.1, 0.2,0.1,0.15]
		...
		self.first = sleep
	def setup_plot(self):
		self.ax.axis([-0.5, self.xronos+0.5, -self.y*1.1, self.y*1.1])
		self.scat = self.ax.scatter(self.xwork, self.ywork, c="RED", s=10, animated=True)
	...
	def update(self, i):
		...
		self.scat = self.ax.scatter(self.xwork, self.ywork, c=random.choice(colors), s=15, animated=True)
		return self.scat,
	def show(self):
		pyplot.show()
